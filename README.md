...
-  인벤토리 아이템 Enter 꾹 눌러서 사용시 잔상이 남음
   -  이번엔 아예 TownScene 차원에서 열릴 당시의 가장 큰 키를 저장해서 지워주기 시도
   -  인벤토리 비주얼 개선
      -  그에 따라 클리어 코드도 수정 
      -  인벤토리의 정확한 값을 계산해서 클리어하는 걸로 개선
      -  초기화 전에 백그라운드 컬러 검정색으로(중요)
      -  인벤토리 늘어나면 인벤토리 생성 포지션이 위로 올라가지만 다시 개선 해주는 작업은 후순위로 미룸,
   -  개선 후 인벤토리에서 포션을 먹을시 HP바가 실시간 반영 되지 않음
      -  예외처리로 그려주기
- LogScene 입장시 이전 Scene 상태 그대로 유지.
  - 배틀 Scene등 구현시 공통적으로 사용할 방식.
  - TownScene 구조 개선 (초기화 분리)
    - Enter()에 있는 **"데이터 생성 코드"**를 Init()으로 옮겨야, 씬을 갔다 와도 맵 상태가 유지
    - Init으로 씬 생성시에만 데이터가 초기화 되니 씬을 이동했다 돌아와도 다시 초기화하지 않음
  - 게임 오버씬 구현
    - 플레이어 체력이 0이 될시 씬 전환.
    - 게임 오버 연출 보여주고 Enter 키 누르면 타이틀 씬 이동
    - 아스키 아트 받아와서 외곽선과 함께 출력
    - 타이틀 씬의 게임 스타트를 new로 바꿔서 게임 새로 시작
      - //SceneManager.AddScene("Town", new TownScene(_player))
      - 주석 처리
- 적 생성
  - 열거형 타입으로 분리
  - 적 종류에 따라 포션처럼 색다르게 맵에 출력.
  - 플레이어의 이동 방향에 적이 있으면 이동을 중단하고 해당 적과 배틀씬으로 이동
  - 적 이름과 적 아스키 아트 출력(색상은 적 색상에 따라 바꿔서 출력(임시))
  - 배틀 연출 후 지면 게임 오버. 이기면 타운씬으로 복귀(미구현)
  - 부딪힌 적에 따라 아스키 아트 색 변경.
- 배틀씬 진입까지 구현 완료, 배틀씬 구현은 아직 미구현
  - 일단 테스트 용으로 Enter를 눌러 이전 씬으로 귀환
